...
import com.gtnewhorizon.gtnhlib.client.renderer.cel.api.util.ColorU8;
...
public class NvidiumCompactChunkVertex implements ChunkVertexType {
    ...
    public static final GlVertexFormat VERTEX_FORMAT = new GlVertexFormat(null, 16);
    ...
    private static int compactLight(int light) {
        ...
        int sky = Math.min(Math.max((light >>> 16) & 0xFF, 8), 248);
        int block = Math.min(Math.max((light >>> 0) & 0xFF, 8), 248);
