...
public class SegmentedManager {
    ...
    /*
     * public long allocFromLargest(int size) {//Allocates from the largest avalible block, this is useful for expanding
     * later on
     * }
     */
    ...
    public int free(long addr) {// Returns size of freed memory
        ...
        } // If there is no previous it means were at the start of the buffer, we might need to merge with block 0 if we
          // are not block 0
        else if (!FREE.isEmpty()) {// if free is not empty it means we must merge with block of free starting at 0
            if (FREE.remove(addr << ADDR_BITS)) {// Attempt to remove block 0, this is very dodgy as it assumes block
                                                 // zero is 0 addr n size
        ...
        } // if there is no next block it means that we have reached the end of the allocation sections and we can
          // shrink the buffer
        else {
            resized = true;
    ...
    public boolean expand(long addr, int extra) {
        ...
        if (iter.hasNext()) {
            ...
            if (extra <= delta) {
                ...
                // else There is exactly enough free space, so removing the free block and updating the allocation is
                // enough
                return true;
            } else {
